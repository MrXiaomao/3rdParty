#include "QNtpClient.h"
#include "qmutex.h"
#include "qudpsocket.h"

QScopedPointer<QNtpClient> QNtpClient::self;
QNtpClient *QNtpClient::instance()
{
    if (self.isNull()) {
        static QMutex mutex;
        QMutexLocker locker(&mutex);
        if (self.isNull()) {
            self.reset(new QNtpClient);
        }
    }
...
QNtpClient::QNtpClient(QObject *parent) : QObject(parent)
{
...
void QNtpClient::setNtpIP(const QString &ntpIP)
{
    if (this->ntpIP != ntpIP) {
        this->ntpIP = ntpIP;
    }
